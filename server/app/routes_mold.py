...
@bp.route('many_products/<int:count>')
def many_products(count):
    categories=Category.query.all()
    catList = [category for category in categories if len(category.products) >= count]
    return jsonify(categories_schema.dump(catList))


    # categories = Product.query.count(count)